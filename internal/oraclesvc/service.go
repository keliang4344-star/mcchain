...
// verifier 为 attestation 注册表，/sign 在签名前必须先通过真机验证。
...
func NewSecureHandlerWithMonitor(priv *secp256k1.PrivKey, addr, pubB64, signToken string, ratePerMin int, verifier AttestationVerifier, monitor *HealthMonitor) http.Handler {
	...
	signFn := func(w http.ResponseWriter, r *http.Request) {
		...
		// Bearer 认证（SEC：常量时间比较，防计时侧信道）
		if signToken != "" {
			got := strings.TrimPrefix(r.Header.Get("Authorization"), "Bearer ")
			if subtle.ConstantTimeCompare([]byte(got), []byte(signToken)) != 1 {
				monitor.MarkFailure()
		...
		// 签名前必须先验证真实设备 attestation（fail-closed：无注册表或验证失败即拒绝）。
		if verifier == nil {
			monitor.MarkFailure()
...
func loadOrGenerateKey(strict bool) (*secp256k1.PrivKey, error) {
	...
	// 临时私钥严禁打印进日志 —— 日志会被 journald/
	// Loki/ELK 采集并长期留存，等于把私钥写进集中式日志系统。
	// 改为只提示固定方法：用 `openssl rand -hex 32` 自行生成后配置。
	priv := secp256k1.GenPrivKey()
	Warnf("未配置 ORACLE_KEY，已生成临时密钥（重启即变，链上验签将失败）。")
	Warnf("请用 `openssl rand -hex 32` 生成固定密钥并通过 ORACLE_KEY 或 ORACLE_KEY_FILE 配置后重启。")
	return priv, nil
}
