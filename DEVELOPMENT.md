...
本文档记录本仓库在 **Windows 沙箱** 下的工具链约定与踩坑点，避免重复踩雷。
所有命令均以 **PowerShell** 运行（Git Bash 直接执行 Windows PE 会 `Exec format error`）。
...
| 组件 | 路径 | 说明 |
|------|------|------|
| Go | `D:\go\bin\go.exe` | Windows Go 1.22.5，`GOROOT=D:\go` |
| GOPATH | `D:\gopath` | 已整体迁移，含 `pkg\mod` / `bin` |
| GOMODCACHE | `D:\gopath\pkg\mod` | 模块缓存 |
...
```powershell
$env:GOROOT="D:\go"
$env:PATH="D:\go\bin;D:\gopath\bin;D:\protoc\bin;" + $env:PATH
$env:GOMODCACHE="D:\gopath\pkg\mod"
```
...
& 'D:\go\bin\go.exe' test ./x/edgeai/... ./x/depin/... ./x/tokenomics/...

# 覆盖率
& 'D:\go\bin\go.exe' test ./x/edgeai/... -cover
```
...
**`ignite generate proto-go` 在本 Windows 沙箱不可用**：Ignite 会下载一个临时 protoc（随机名、无 `.exe` 扩展名）执行，报 `executable file not found in %PATH%`，且无视系统 PATH 里的 protoc。

改用 **手动 protoc**（已验证可用）。示例（在 `mcchain` 仓库根目录执行，需先 `Set-Location`）：
...
$deps = @(
  "-I", "proto",
  "-I", "D:\gopath\pkg\mod\github.com\cosmos\cosmos-sdk@$($cosmosSdkVer)\proto",
  "-I", "D:\gopath\pkg\mod\github.com\cosmos\gogoproto@v1.4.10",
  "-I", "D:\gopath\pkg\mod\github.com\cosmos\cosmos-proto@v1.0.0-beta.2\proto",
  "-I", "D:\gopath\pkg\mod\github.com\cosmos\gogo\googleapis@v1.4.1",
  "-I", "D:\gopath\pkg\mod\github.com\ignite-hq\cli\ignite\pkg\protoc\data\include"
)
...
本机 `D:\gopath\bin\protoc-gen-gocosmos` 在某些沙箱会话中会被清掉，导致 `protoc ... --gocosmos_out` 报
`protoc-gen-gocosmos 不是内部或外部命令`。直接从模块缓存源码重建即可（gogoproto 已在 go.mod 锁定）：

```powershell
# 在 mcchain 仓库根目录
& 'D:\go\bin\go.exe' build -o protoc-gen-gocosmos.exe github.com/cosmos/gogoproto/protoc-gen-gocosmos
# 然后 protoc 用 --plugin 显式指向刚生成的 exe（注意用 Windows 绝对路径）
protoc -I proto -I "D:\gopath\pkg\mod\github.com\cosmos\gogoproto@v1.4.10" `
  --plugin=protoc-gen-gocosmos=$PWD/protoc-gen-gocosmos.exe `
...
- `ignite chain serve` / `ignite generate proto-go` 在 Windows 沙箱不可用 → 用 `go build` + 手动 protoc。
