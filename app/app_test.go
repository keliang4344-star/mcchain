...
// 避免 app.New 因缺失公钥而 panic（生产强制逻辑；生产节点必须配置公钥）。
...
// TestTokenomicsMaccPerms 验证 app.go 模块账户权限划分（五池模型）：
//   - tokenomics 持有 Minter（唯一发行入口）；
//   - depin 不再持有 Minter/Burner（仅 {Staking}），遵循最小权限，同时托管设备激励池资金；
...
func TestTokenomicsMaccPerms(t *testing.T) {
	...
	// depin 必须保留注册（托管设备激励池），但不得持有 Minter/Burner（仅 Staking）。
	depinPerms, ok := maccPerms[depinmoduletypes.ModuleName]
	require.True(t, ok, "depin must remain registered in maccPerms")
	require.NotContains(t, depinPerms, authtypes.Minter, "depin must NOT hold Minter")
	require.NotContains(t, depinPerms, authtypes.Burner, "depin must NOT hold Burner (its BankKeeper interface has no BurnCoins; remove redundant permission)")
	require.ElementsMatch(t, []string{authtypes.Staking}, depinPerms)
...
// TestTokenomicsInitGenesisOrder 验证 genesis 顺序铁律：
