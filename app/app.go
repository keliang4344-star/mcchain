...
var (
	...
	//
	// 列表由 moduleBasics() 组装：条件编译模块（当前是 x/wasm）必须与 ModuleManager
	// 同口径注册，否则 BasicManager 与 mm.Modules 会漂移 —— 注记见 moduleBasics()。
	...
	maccPerms = map[string][]string{
		...
		// depin 移除 Minter（不再自铸），且不持有 Burner 权限（其 BankKeeper
		// 接口已无 BurnCoins，移除冗余权限以遵循最小权限原则）；仅保留 Staking。
		depinmoduletypes.ModuleName:      {authtypes.Staking},
		...
		// 协议自有流动性账户：持有创世池的 LP 份额（pool1 denom）。
		// 必须注册——SDK v0.47.14 bank.SendCoinsFromModuleToModule 在收款方是
		// 模块账户时会校验其存在性（auth.GetModuleAccount 返回 nil 即
		// panic "module account community does not exist"）。缺这一条时，
		// 一旦创世按白皮书 §24 预拨 USDT 建立 MC/USDT 池，InitChain 当场 panic。
		dexmoduletypes.CommunityModuleName:         nil,
...
// moduleBasics 返回 BasicManager 的模块列表（含条件编译模块）。
//
// x/wasm 仅在 CGO 构建可用（wasmvm 可链接）。若它只被塞进
// ModuleManager（app.mm.Modules），却没有同步进 ModuleBasics，两条路径因此漂移：
//
//	· DefaultGenesis 走 ModuleBasics → CGO 构建下 `init` 产出的 genesis 里
//	  **没有 app_state.wasm 段**，而 mm.InitGenesis 照旧调用 wasm.InitGenesis
//	  去反序列化空数据（params 全为零值）；
//	· validate-genesis 也看不到 wasm 段，非法参数被一路带到 InitChain 才炸。
//
// 现在 BasicManager 与 ModuleManager 共用同一个条件编译开关（wasmAppModuleBasic），
// 非 CGO 构建两边都没有 wasm，CGO 构建两边都有。
func moduleBasics() []module.AppModuleBasic {
	list := []module.AppModuleBasic{
		auth.AppModuleBasic{},
	...
	if wb := wasmAppModuleBasic(); wb != nil {
		list = append(list, wb)
	}
	return list
}
...
//
// 铸币铁律：MC 总量硬顶 1e15 umc（10 亿 MC），由 tokenomics 在创世
...
func New(
	...
	*/
	...
	// 注册软件升级处理器：无处理器时，通过的 SoftwareUpgrade 治理提案会在目标高度
	// 停机且无迁移可执行，导致不可逆停链。默认处理器运行全部模块迁移。
	app.RegisterUpgradeHandlers()

	// 升级存储加载器（Store Loader）。缺少它时，任何「新增/删除模块 store」
	...
	// 接线顺序铁律：PhonenodeKeeper MUST be created before DepinKeeper
	...
	// edgeai：依赖 phonenode (IsAttested/SlashIfBad) + bank (payout)，genesis 排在 depin/phonenode 后
	...
	// tokenomics：唯一持 Minter 的「发行与分配总账」模块。
	...
	// 注意：dexModule 不在这里构造 —— 其 BeginBlock 发放的 LP 激励走设备池释放
	// 闸门，必须等下方 DexKeeper.SetDepinKeeper 完成之后再构造。
	...
	// phonenode 的「节点资本津贴」从设备池出账，必须经 depin 的每日线性释放
	// 闸门。depin 依赖 phonenode，故只能在 depin 构造完成后反向回填。
	app.PhonenodeKeeper.SetDepinKeeper(app.DepinKeeper)

	// dex 的 LP 激励从设备池出账，必须经 depin 的每日
	// 线性释放闸门。dex 与 depin 互不依赖，故在 depin 构造完成后反向回填。
	app.DexKeeper.SetDepinKeeper(app.DepinKeeper)

	// AppModule 以「值」持有 keeper，RegisterServices
	// 用的是 am.keeper 这份拷贝。若 depinModule / edgeaiModule 在上面的
	// SetReferralKeeper 之前就被构造，拷贝里的 referralKeeper 永远是 nil；而两处
	// 调用点都做了 `if referralKeeper != nil` 的判空保护，于是 10 代推荐佣金在真实
	// 链上「静默地」从未触发过一次——测试里直连 app.DepinKeeper 反而看不出来。
	// 因此这两个 AppModule 必须在接线完成之后再构造，拷贝到的才是带 hook 的 keeper。
	// 同理，phonenodeModule 的 BeginBlock 走 am.keeper 分发节点资本津贴，必须
	// 在 SetDepinKeeper 之后构造，否则副本里的释放闸门是 nil、津贴会被 fail-closed 扣住。
	// dexModule 的 BeginBlock 走 am.keeper 发放 LP 激励，同理必须在
	// DexKeeper.SetDepinKeeper 之后构造。
	...
	app.mm = module.NewManager(
		...
		// 传入硬编码零通胀计算函数（传 nil 会退回 SDK 默认 ~13% 通胀）。
	...
	app.mm.SetOrderBeginBlockers(
		...
		// tokenomics 必须在 distribution 之前执行 BeginBlock。
		// 否则 distribution.BeginBlocker 的 AllocateTokens 会先把 fee_collector
		// 的全部余额转入 distribution 模块账户，导致 tokenomics 的 gas 7% 销毁 +
		// 10% 安全池回流读到 0 余额而恒不触发，通缩模型核心叙事落空。
		...
		// mint 必须保留在 BeginBlockers 中——SDK 的 assertNoForgottenModules
	...
	// 创世顺序铁律断言：tokenomics → depin → phonenode → edgeai。
	...
	// Wrap the default ante handler with a chain-wide minimum self
	...
	//
	// 装饰器会递归解包 authz.MsgExec / gov / group 提案携带的
	// 内层消息，因此必须注入 interfaceRegistry（用于在 GetMessages() 缓存缺失时
	// 自行解包 Any）。不注入的话遇到嵌套消息会 fail-closed 而非放行。
	msd := NewMinSelfDelegationDecorator(app.interfaceRegistry)
	...
	// 生产预言机强制：主网/生产必须启用 TeeOracle 做链上真实验签，
	...
	// 链下签名服务见 cmd/oracle（签名前强制验证真实设备硬件 attestation）；公钥由运营离线保管。
...
func (app *App) InitChainer(ctx sdk.Context, req abci.RequestInitChain) abci.ResponseInitChain {
	...
	// ---- post-genesis overrides (must run AFTER InitGenesis) ----

	// force the staking BondDenom to "umc" regardless of what the genesis
	// file may contain, so it stays consistent with the genesis account set.
	p := app.StakingKeeper.GetParams(ctx)
	p.BondDenom = "umc"
	app.StakingKeeper.SetParams(ctx, p)

	// 固定总量链——tokenomics 模块一次性铸造并强约束 cap(1e15 umc)，
	// 链上绝不允许二次通胀。mint 模块默认 inflation≈13%，若持有 Minter 权限
	// 便可绕过 tokenomics 的 cap 直接铸币，故此处在创世把通胀参数清零。
	//
	// InitChainer 仅在**创世**执行一次，并非"每次启动兜底"。
	// 参数清零因此只是第一道防线——真正的
	...
	// gov 否决（veto）押金默认会经 bank.BurnCoins 真实销毁 umc，
	// 这构成「白皮书 §24 三项销毁清单」之外的第 4 条链上销毁路径，并破坏
	// 「全链销毁统一进黑洞、总量 10 亿恒定不变」的不变量。显式关闭 veto 销毁，
	// 否决提案的押金原路退回提案人，链上不再烧币。
	gp := app.GovKeeper.GetParams(ctx)
	gp.BurnVoteVeto = false
	if err := app.GovKeeper.SetParams(ctx, gp); err != nil {
		panic(fmt.Sprintf("tokenomics: failed to disable gov veto burn: %v", err))
	}

	// 将 CosmWasm 合约上传/实例化权限收敛到治理账户，禁止任意地址
	...
	// /C: genesis validators are created by InitGenesis and therefore bypass
	...
	// 定义于 app/ante.go，白皮书 §A.6 同值。此处直接引用常量，
	// 避免注释与代码二次漂移。
	...
	// x/edgeai 的争议仲裁人（Arbitrator）默认是空串。留空时 ResolveDispute
	// 没有任何地址能通过鉴权——被质疑的任务将永远停在争议态，其托管金既退不回
	// 需求方也发不给提交者，等于每一笔争议都在链上锁死一笔钱。
	// 开发/测试链允许留空以便一键起链；生产链创世必须显式配置团队多签地址，
	// 在此 fail-closed，避免带着「争议系统不可用」的状态上主网。
	if err := app.EdgeaiKeeper.GetParams(ctx).ValidateOperational(); err != nil {
		if isProductionChainID(ctx.ChainID()) {
			panic(fmt.Sprintf("genesis rejected: %v (chain-id=%s)", err, ctx.ChainID()))
		}
		ctx.Logger().Error("edgeai params not production-ready", "err", err, "chain_id", ctx.ChainID())
	}

	// 区块 gas 上限在链上 fail-closed 兜底。
	//
	// CometBFT 的 consensus_params.block.max_gas 默认是 -1（不限）。一旦创世沿用该默认值，
	// baseapp.GetMaximumBlockGas 会返回 0 并改用 InfiniteGasMeter —— 单笔交易即可无限消耗
	// gas 占满整个区块，构成出块失控 / 全网停摆的零成本 DoS 路径。
	//
	// 该上限若只由离链脚本保证（server_setup.sh 与 scripts/make_genesis.py 写入 1e8）：
	// 任何绕开脚本手工拼装创世的部署都会静默失守，而且从链上看不出任何异常。
	// 这里把兜底移到链上——创世若未设或仍为「不限」，直接钳制到 DefaultBlockMaxGas。
	if cp := app.enforceBlockMaxGas(ctx); cp != nil {
		// 回写响应：baseapp.InitChain 会把 res.ConsensusParams 透传给 CometBFT。
		// 只改应用侧 paramstore 是不够的——共识侧仍会按 -1 打包区块，两侧口径将不一致。
		res.ConsensusParams = cp
	}

	// 创世真实总供应不得击穿硬顶。
	//
	// tokenomics 的不变量只约束「自己铸了多少」（minted_supply ≤ cap），
	// 并不约束 bank 的总供应。而 `add-genesis-account` 是凭空记账：只要主网创世
	// 给任何账户拨了一笔不在五池之内的余额，总供应就会 > 1e15 umc，
	// 「总量 10 亿 MC 恒定」这条最核心的承诺当场失效 —— 而且一旦出块就不可逆
	// （只能靠治理迁移修）。
	//
	// 这类错误 validate-genesis 查不出来（它只校验各模块 schema，
	// 看不到跨模块的真实账本），devnet 实测就是 1.1e15（devnet 有意在五池之外
	// 给验证人注资）。因此必须在 InitGenesis 全部完成之后，对真实账本核一次。
	app.assertGenesisSupplyCap(ctx)

	return res
}

// assertGenesisSupplyCap 核对创世总供应与硬顶的关系。
//
// 生产链：超过硬顶直接拒绝启动（宁可起不来，也不能带着一份「总量已被击穿」的
// 账本上线）；低于硬顶只告警（销毁类操作理论上可能让余额略低于 cap，
// 那是通缩方向，不构成承诺失效）。
// 非生产链：一律只告警，保证 devnet / CI 能正常起链。
func (app *App) assertGenesisSupplyCap(ctx sdk.Context) {
	supply := app.BankKeeper.GetSupply(ctx, tokenomicsmoduletypes.DefaultDenom).Amount
	cap := sdk.NewIntFromUint64(tokenomicsmoduletypes.TotalSupplyCap)
	if supply.Equal(cap) {
		return
	}
	detail := fmt.Sprintf("genesis total supply %s umc != hard cap %s umc", supply, cap)
	if supply.GT(cap) {
		if productionGenesisRequired(ctx.ChainID()) {
			panic("genesis rejected: " + detail +
				" — some genesis account was funded outside the five pools; this permanently breaks the fixed 1B MC promise (chain-id=" +
				ctx.ChainID() + ")")
		}
		ctx.Logger().Error("genesis supply cap exceeded (non-production chain, continuing)",
			"detail", detail, "chain_id", ctx.ChainID())
		return
	}
	if productionGenesisRequired(ctx.ChainID()) {
		// 供应低于硬顶同样是违约：tokenomics 创世会在 InitGenesis 一次性铸满 1e15，
		// 正常路径不可能不足。出现不足说明铸币/拨付被跳过（例如自定义创世绕过了
		// tokenomics），创世必须失败而不是带着「少发的总量」出块 —— 那会让
		// 白皮书里的总量承诺与链上账本从此不一致，且无法回滚。
		panic("genesis rejected: " + detail +
			" — tokenomics.genesis did not mint the full hard cap; a chain must start with exactly 1e15 umc (chain-id=" +
			ctx.ChainID() + ")")
	}
	ctx.Logger().Error("genesis total supply below hard cap", "detail", detail, "chain_id", ctx.ChainID())
}

// productionGenesisRequired 报告本次起链是否必须按生产口径做创世硬校验。
//
// 判据与 x/tokenomics 的创世密钥闸门保持一致（创世密钥闸门的同一条原则）：
// chain-id 是运维自己起的字符串，**不能**作为唯一判据，必须叠加一个可静态审计的
// 显式声明（MC_REQUIRE_REAL_GENESIS_KEYS=1，写在 systemd unit / 部署脚本 / 验收脚本里）。
//
// 两个判据是「或」关系：
//   - isProductionChainID 是黑名单式（含 test/dev/local/sim 之外的都算生产），
//     因此默认方向是 fail-closed —— 起个随意名字的链也按生产口径拦；
//   - 显式开关用于那些 chain-id 里恰好带了 "dev/local" 字样、但其实是生产部署的场景。
func productionGenesisRequired(chainID string) bool {
	return isProductionChainID(chainID) ||
		os.Getenv(tokenomicsmoduletypes.RequireRealGenesisKeysEnv) == "1"
}

// DefaultBlockMaxGas 是创世未显式设定时链上强制的区块 gas 上限。
//
// 取值与离链创世脚本（server_setup.sh、scripts/make_genesis.py）写入的 100000000 一致，
// 确保「用脚本建链」与「手工建链」落到同一个上限，不产生第二套口径。
const DefaultBlockMaxGas int64 = 100_000_000

// enforceBlockMaxGas 校验并在必要时钳制创世的区块 gas 上限。
//
// 返回值语义：仅当确实修正过参数时返回非 nil（供 InitChainer 回写 ResponseInitChain
// 同步给 CometBFT）；创世本身已给出有界上限时返回 nil，不触碰共识侧状态。
func (app *App) enforceBlockMaxGas(ctx sdk.Context) *tmproto.ConsensusParams {
	cp, err := app.ConsensusParamsKeeper.Get(ctx)
	if err != nil {
		// 读不出共识参数说明创世状态已损坏，继续起链只会带着未知的 gas 上限运行。
		panic(fmt.Sprintf("genesis rejected: cannot read consensus params: %v", err))
	}
	if cp == nil {
		cp = &tmproto.ConsensusParams{}
	}

	// max_gas > 0 表示创世已显式设定有界上限，无需干预。
	if cp.Block != nil && cp.Block.MaxGas > 0 {
		return nil
	}

	// 走到这里意味着 Block 缺失、max_gas 为 0，或为 -1（不限）——三者都会导致
	// 区块 gas 无界。补齐 Block 时 MaxBytes 必须取 CometBFT 官方默认值：
	// ConsensusParams.Update 会用返回值整体覆盖 Block 段，若 MaxBytes 留 0
	// 将写出一个非法的共识参数并让节点在校验阶段拒绝启动。
	defaults := tmtypes.DefaultConsensusParams()
	previous := int64(0)
	if cp.Block == nil {
		cp.Block = &tmproto.BlockParams{MaxBytes: defaults.Block.MaxBytes}
	} else {
		previous = cp.Block.MaxGas
		if cp.Block.MaxBytes <= 0 {
			cp.Block.MaxBytes = defaults.Block.MaxBytes
		}
	}
	...
	ctx.Logger().Error(
		"genesis block.max_gas was unbounded; clamped on-chain to keep block gas metered",
		"previous", previous, "enforced", DefaultBlockMaxGas, "chain_id", ctx.ChainID(),
	)
	return cp
}

// isProductionChainID 判断 chain-id 是否为生产网络。
//
// 默认判定为「是」——安全检查的失败方向必须是拦截而非放行。只有明确带有
// test/dev/local/sim/simulation 标记的链才被视为非生产环境并降级为告警。
func isProductionChainID(chainID string) bool {
	id := strings.ToLower(chainID)
	for _, marker := range []string{"test", "dev", "local", "sim"} {
...
// SDK 的 store 版本在升级时若与 app 注册的 store key 集合不一致，
...
// 若没有任何 SetUpgradeHandler，一旦 SoftwareUpgrade 治理提案通过，
...
// EnableDexSettlementAtUpgrade 注册一个一次性升级处理器：在指定升级高度开启 DEX 结算通道，
// 并把结算 Authority 设为 operatorAddr。
//
// x/dex 的结算通道默认 Authority = 治理模块账户，而治理模块账户
// 无法签名交易，因此 Submit/FinalizeSettlementBatch 对任何人都不可用 —— 创世会显式
// 把 Halted 置位（链上状态说真话：「未启用」），真正的开启动作只能发生在一次
// 软件升级中。若只提供 EnableSettlement 而没有调用它的地方，那它自己也会变成
// 另一种死代码；本方法即为它的正式接线点。
//
// 用法（运营侧）：
//
//	app.RegisterUpgradeHandlers()            // 已由 NewApp 调用
//	app.EnableDexSettlementAtUpgrade("mcchain-v2", operatorAddr)
//
// 参数校验前置在注册时完成：地址非法或指向模块账户会立即返回错误，
// 避免把一个注定死锁的配置写进升级处理器。
func (app *App) EnableDexSettlementAtUpgrade(upgradeName, operatorAddr string) error {
	if upgradeName == "" {
		return fmt.Errorf("dex settlement: upgrade name must not be empty")
	}
	if app.UpgradeKeeper.HasHandler(upgradeName) {
		return fmt.Errorf("dex settlement: upgrade handler %q already registered", upgradeName)
	}
	if err := dexmodulekeeper.ValidateSettlementAuthority(operatorAddr); err != nil {
		return fmt.Errorf("dex settlement: %w", err)
	}
	app.UpgradeKeeper.SetUpgradeHandler(
		upgradeName,
		func(ctx sdk.Context, plan upgradetypes.Plan, fromVM module.VersionMap) (module.VersionMap, error) {
			...
			return vm, app.DexKeeper.EnableSettlement(ctx, operatorAddr)
		},
	)
	return nil
}
