...
// 不得实现为
//
//	CalcSwapOutput(reserveIn, reserveOut, amountIn-nonLPFee, 0)
//
// 即定价时费率写死为 0，等价于「零手续费成交」。后果是 LP 手续费份额从未沉淀进
// 储备——恒定乘积 k 不增长，做市商真实收益恒为 0，而交易者按无费价格成交，池子
// 被按更优价格持续抽走深度。
//
// 正确口径（与 Uniswap 一致）：
//   - 定价基数：全额 amountIn 参与、按 feeRateBps 扣费 → 交易者承担全额手续费；
//   - 池内实收：amountIn - nonLPFee（非 LP 份额由 ProcessSwapFee 外送销毁/国库）；
//   - 两者之差恰为 LP 份额，留存在储备中使 k 增长，即做市商收益。
func CalcSwapOutputWithPoolFee(reserveIn, reserveOut, amountIn sdk.Int, feeRateBps uint32) (amountOut, nonLPFee sdk.Int) {
	...
	return CalcSwapOutput(reserveIn, reserveOut, amountIn, feeRateBps), nonLPFee
}
