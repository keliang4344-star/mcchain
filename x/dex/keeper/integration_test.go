...
func TestDEX_CreatePoolAndSwap_FullFlow(t *testing.T) {
	...
	// regression: the trader must be charged the FULL fee (pricing base =
	// amountIn net of feeRateBps), while only the non-LP share leaves the
	// reserve. Previously the pricing call hardcoded fee=0, which handed the
	// trader a zero-fee quote and left LPs earning nothing.
	...
	expectedOut := CalcSwapOutput(reserveA, reserveB, swapAmount, 30)
	...
	// reserveIn' = reserveIn + amountIn - nonLPFee ; reserveOut' = reserveOut - amountOut
	...
	// core invariant: k must STRICTLY grow — that growth is the LP yield.
	// With the old fee=0 pricing this assertion fails (k stays flat/shrinks).
	kBefore := reserveA.Mul(reserveB)
	kAfter := newRA.Mul(newRB)
	require.True(t, kAfter.GT(kBefore), "constant product k must grow by the LP fee share (LP yield)")
...
func TestDEX_RemoveLiquidity_Basic(t *testing.T) {
	...
	reserveA, _ := sdk.NewIntFromString(pool.ReserveA)
	reserveB, _ := sdk.NewIntFromString(pool.ReserveB)
	// TotalLp 里有 MinimumLiquidity(1000) 份永久锁在黑洞，建池者只拿到
	// TotalLp-1000 份。赎回其中一半后，池中留存略高于一半（多出的正是那 1000
	// 份锁定份额对应的储备），偏差上界为 MinimumLiquidity 本身。
	require.InDelta(t, int64(50_000000), reserveA.Int64(), types.MinimumLiquidity)
	require.InDelta(t, int64(50_000000), reserveB.Int64(), types.MinimumLiquidity)
...
func TestDEX_CreatePool_MaxPoolLimit(t *testing.T) {
	...
	// 每个池的首存都必须大于 MinimumLiquidity，否则会先撞
	// ErrInsufficientLiquidity 而测不到 MaxPools 这条边界。
	// Create pool 1
	_, err := k.CreatePool(ctx, "adenom", "bdenom", sdk.NewInt(1_000_000), sdk.NewInt(1_000_000), 30, lp, 0)
	require.NoError(t, err)

	// Create pool 2
	_, err = k.CreatePool(ctx, "cdenom", "ddenom", sdk.NewInt(1_000_000), sdk.NewInt(1_000_000), 30, lp, 0)
	require.NoError(t, err)

	// Pool 3 → nextPoolID=3, MaxPools=2 → should fail
	_, err = k.CreatePool(ctx, "edenom", "fdenom", sdk.NewInt(1_000_000), sdk.NewInt(1_000_000), 30, lp, 0)
...
func TestDEX_GetAllPools(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		// 首存需大于 MinimumLiquidity。
		_, err := k.CreatePool(ctx, denom1, denom2,
			sdk.NewInt(int64(1_000_000*(i+1))), sdk.NewInt(int64(1_000_000*(i+1))), 30, lp, 0)
		require.NoError(t, err)
	}
