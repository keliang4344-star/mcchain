...
func (k Keeper) SwapExactIn(
	...
	// Price the swap through the shared AMM helper: the trader pays the full
	// fee (pricing base = amountIn net of feeRateBps), while the reserve only
	// gives up the non-LP share (burn + treasury, routed by ProcessSwapFee).
	// The difference is the LP share, which stays in the reserve and makes
	// x*y=k grow — that growth *is* the liquidity provider's yield.
	// EstimateSwap calls the exact same helper, which is what guarantees a
	// quote can never diverge from execution.
