...
type (
	...
	// 唯一持 Minter 的模块；无 Msg service，运行期不增发/不销毁。
	Keeper struct {
...
// 否则 panic（总量固化，不可突破）。铸造后累加并持久化 minted_supply。
...
// 注意这里不再调用 bank.BurnCoins —— 全链销毁统一去向为黑洞地址（总量固化）：
...
//
// 国库资金不由任何模块凭空生成，只能像这样由真实业务收入转入（零新印）。
