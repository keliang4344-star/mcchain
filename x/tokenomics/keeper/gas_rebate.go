...
func (k Keeper) RebateGasFeesToSecurity(ctx sdk.Context) error {
	...
	// 比例计算全程走 sdk.Int。先 .Uint64 再乘以 bps 有两条隐患：余额超出
	...
	//
	// 修正：销毁额按 fee_collector 余额计算，扣款账户必须同为 fee_collector。
	// 朴素实现从 tokenomics 模块账户扣款（余额恒为 0），必然失败且 return 阻断了后续
	...
	// 10% of gas fees rebated to the staking-security pool (安全池闭环).
