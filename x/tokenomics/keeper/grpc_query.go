...
// Supply 实现 gRPC Query/Supply（总量查询）。
...
// Allocations 实现 gRPC Query/Allocations（分配查询，实时读取各池余额）。
...
// Release 实现 gRPC Query/Release（释放进度查询，实时计算）。
...
// QueryRelease 返回团队池释放进度。进度基于曲线元数据 + 当前区块时间实时计算（不改状态）。
