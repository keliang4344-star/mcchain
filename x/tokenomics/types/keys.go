...
const (
	...
	// 链上常量 + Genesis 校验双保险；不进 params subspace、不治理可改。
	TotalSupplyCap = uint64(1e15)

	// TeamMultisigThreshold 团队多签阈值（3-of-5）。
	...
	//   2) DEX 手续费销毁：每笔撮合手续费（0.30%）的 50%，即成交额的 0.15%
	//      （FeeBurnBps = 5000，x/dex/keeper/fee.go）→ 黑洞；另 50% 归流动性提供者；
...
func init() {
	...
	//
	// 这三个派生在包 init 里完成，早于日志系统就绪。
	// 一旦 env 里填了非法 bech32，进程只会吐一行没有上下文的 panic 堆栈就退出，
	// 运维无法判断是三个变量里的哪一个填错了。故 panic 文案必须自带变量名。
	EarlyDevPubKey, EarlyDevAddress = resolveFoundationKey("MC_EARLY_DEV_PUBKEY", earlyDevPubKeyOverride, 0x21)
	FoundationOpsPubKey, FoundationOpsAddress = resolveFoundationKey("MC_FOUNDATION_OPS_PUBKEY", foundationOpsPubKeyOverride, 0x22)
	FoundationVestingPubKey, FoundationVestingAddress = resolveFoundationKey("MC_FOUNDATION_VESTING_PUBKEY", foundationVestingPubKeyOverride, 0x23)
}
...
//
// label 是来源标识（env 变量名 / 生成文件名），用于在 init 阶段的 panic 文案里
// 精确定位是哪一处覆写配错了 —— 这个阶段还没有日志，panic 是唯一的输出通道。
func resolveFoundationKey(label, override string, seed byte) (cryptotypes.PubKey, sdk.AccAddress) {
	if override == "" {
		return derivedPlaceholder(seed)
	}
	_, bz, err := bech32.DecodeAndConvert(override)
	if err != nil {
		panic(fmt.Sprintf("tokenomics: %s is not valid bech32 (value=%q): %v — fix the env var or x/tokenomics/types/foundation_addrs_gen.go before starting the node", label, override, err))
	}
	pk, err := legacy.PubKeyFromBytes(bz)
	if err != nil {
		panic(fmt.Sprintf("tokenomics: %s is not a valid secp256k1 pubkey (value=%q): %v — fix the env var or x/tokenomics/types/foundation_addrs_gen.go before starting the node", label, override, err))
	}
	return pk, sdk.AccAddress(pk.Address())
}

// RequireRealGenesisKeysEnv 是主网创世的显式密钥闸门开关。
//
// 设为 "1" 时，若团队多签 / 基金会拨付地址仍走源码可推导的占位派生，创世直接
// 失败（链起不来），而不是「打一行日志继续跑」。
//
// 为什么需要它：仅靠 chain-id 含 "mainnet" 子串来判定主网是不成立的 —— chain-id
// 是运维自己起的字符串，改个名就能让闸门静默失效，而 1.75 亿 MC 会落到任何读过
// 源码的人都能还原的私钥上。显式开关与命名无关，且能被 systemd unit / 部署脚本 /
// 上线验收脚本静态断言，是唯一可审计的判据。
const RequireRealGenesisKeysEnv = "MC_REQUIRE_REAL_GENESIS_KEYS"

// IsMainnetChainID 报告 chain-id 是否看起来像主网（第二道保险，非唯一判据）。
func IsMainnetChainID(chainID string) bool {
	return strings.Contains(strings.ToLower(chainID), "mainnet")
}
...
func buildTeamPubKeys() []cryptotypes.PubKey {
	if len(teamPubKeyStrings) == 5 {
		...
		for i, s := range teamPubKeyStrings {
			_, bz, err := bech32.DecodeAndConvert(s)
			if err != nil {
				panic(fmt.Sprintf("tokenomics: team multisig pubkey #%d (%q) is not valid bech32: %v — fix x/tokenomics/types/team_pubkeys_gen.go before starting the node", i+1, s, err))
			}
			pk, err := legacy.PubKeyFromBytes(bz)
			if err != nil {
				panic(fmt.Sprintf("tokenomics: team multisig pubkey #%d (%q) is not a valid secp256k1 pubkey: %v — fix x/tokenomics/types/team_pubkeys_gen.go before starting the node", i+1, s, err))
			}
			pks = append(pks, pk)
		}
