...
//
// 这里刻意用 encoding/json 而不是 cdc（ProtoCodec）：本模块的 GenesisState 是
// 手写 Go 结构体（无 protobuf 描述），jsonpb 无法解析。InitGenesis 走的是同一个
// encoding/json，两侧口径一致 —— 一致性才是这里的关键（见 tokenomics/module.go
// 的创世校验注释：验收与起链必须用同一个编解码器，否则验收就是假 PASS）。
