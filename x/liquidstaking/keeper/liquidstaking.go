...
//
// 空池与「被罚没归零的池」必须区分。朴素实现对两者都返回 1.0，于是一个
// 已被 slash 清零、ulmc 实际一文不值的池子，对外仍报价 1 umc/ulmc——
// 前端、DEX 报价、二级市场都会照此定价，直接坑接盘方。份额仍在流通但
// 抵押物归零时，正确答案是 0。
...
func (k Keeper) AccrueRewards(ctx sdk.Context) (uint64, error) {
	...
	for _, del := range delegations {
		...
		// amount 来自 distribution 模块，Int.Uint64 在超界时 panic；
