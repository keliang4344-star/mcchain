...
// TestWipedOutPoolQuotesZero 是池被清空后报价归零的回归测试。
//
// 池被罚没归零、份额仍在流通时，朴素实现与「空池」一样报价 1.0——前端、DEX
// 报价与二级市场都会照此给一堆一文不值的 ulmc 定价。抵押物归零时唯一诚实的
// 答案是 0，且此状态下禁止新入金（ErrPoolWipedOut）。
func TestWipedOutPoolQuotesZero(t *testing.T) {
	f := setupLiquidStaking(t)

	// 空池：首个存款人 1:1，报价 1.0。
	require.True(t, f.k.ExchangeRate(f.ctx).Equal(sdk.OneDec()), "empty pool quotes 1.0")

	// 份额仍在，抵押物被罚没清零。
	ps := f.k.GetPoolState(f.ctx)
	ps.TotalSharesUlmc = 100_000_000
	ps.TotalBondedUmc = 0
	f.k.SetPoolState(f.ctx, ps)

	require.True(t, f.k.ExchangeRate(f.ctx).IsZero(), "wiped-out pool must quote 0, not 1.0")

	// 该状态下新入金必须被拒绝，否则新人本金会补贴给归零的老份额。
	val := f.staking.addValidator(t, false)
	user := f.fundedAddr(t, 500_000_000)
	_, err := f.k.LiquidStake(f.ctx, user, val, 10_000_000)
