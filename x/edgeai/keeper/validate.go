...
// detectCheatByConsensus 实现链上作弊自动检测：对同一任务的多节点 pending 结果做 ResultHash 一致性投票。
//
// 机制：
...
//
// 本函数会调用 SlashIfBad → RecordSlash，
// 向链上 slash 记录列表按顺序追加条目，属于会写入共识状态的操作。
// 若直接 range 两个 Go map（pendingByTask / hashGroups）来驱动这些写入，
...
func (k Keeper) AllResults(ctx sdk.Context) []*Result {
	...
	for ; it.Valid(); it.Next() {
		...
		if err := k.cdc.Unmarshal(it.Value(), &r); err != nil {
			// 同 state.go —— BeginBlock 扫描路径上的状态
			// 损坏必须自愈（删条目 + 告警），不能 panic，否则永久停链。
			dropCorruptEntry(ctx, store, "result", it.Key(), err)
			continue
		}
		rr := r
		out = append(out, &rr)
...
// dropCorruptEntry 处理一条无法反序列化的状态条目：告警 + 删除，链继续出块。
//
// 为什么删而不是留：留着会让这条损坏条目在每一块都被重新扫到、反复报错，
// 既白耗扫描预算又淹没有效告警。删除是全网确定性操作（所有节点读到同一份
// 损坏数据），不引入分歧。被删条目对应的资金仍留在模块托管账户/争议流程里，
// 不会凭空消失。
func dropCorruptEntry(ctx sdk.Context, store prefix.Store, kind string, key []byte, err error) {
	ctx.Logger().Error("edgeai: corrupt state entry dropped (self-heal)",
		"kind", kind, "key", string(key), "err", err.Error())
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"edgeai.CorruptStateDropped",
		sdk.NewAttribute("kind", kind),
		sdk.NewAttribute("key", string(key)),
...
// BeginBlock 实现「贡献即挖矿」经济闭环的结算（需求方付费 / escrow 模型）：
//   - 扫描所有 pending 结果；
...
func (k Keeper) BeginBlock(ctx sdk.Context) {
	...
	for _, r := range results {
		...
		//
		// 必须把被跳过的结果落为
		// Rejected —— SetResult 是 pending 索引唯一的清理点，只 continue 不写状态
		// 会让这条结果永久留在 pending 索引里：每轮结算扫描都会重新读到它，
		// 白耗 MaxPendingResultScanPerBlock=512 的扫描预算与 MaxTasksPerBlock=20
		// 的结算预算。触发条件平凡（任何任务有 ≥2 个不同哈希的提交结果即产生），
		// 累积后结算吞吐随历史任务数单调衰减。
		// 注：cheat 路径已由 rejectResultsForTask 统一清理，正常结算路径同样需要清理。
		if task.Status == types.TaskStatusDone {
			r.Status = types.ResultStatusRejected
			if err := k.SetResult(ctx, r); err != nil {
				ctx.Logger().Error("edgeai: reject superseded result failed",
					"task_id", r.TaskId, "submitter", r.Submitter, "err", err.Error())
			}
			continue
		}
		...
		// 争议/窗口结算判定：
		//   - 争议已裁定 cheat → 跳过拨付（提交者已在裁定时 slash，结果已随案拒绝）
		...
		if dispute != nil {
			...
			case "open":
				if dispute.OpenedAtBlock > 0 && (ctx.BlockHeight()-dispute.OpenedAtBlock) >= params.DisputePeriodBlocks {
					// 超时默认结案方向必须由链上证据决定。
					// 假设「争议仅在重算哈希不一致时开启」并不成立，而 MsgOpenDispute
					// 无任何证据门槛时 —— 无证据争议熬过窗口即被自动判 cheat，
					// 无辜提交者被 slash + 托管金回收。现在：
					//   - 存在链上重算证据且与提交哈希不一致 → 维持证据、判 cheat；
					//   - 无证据 → 乐观判 honest（恢复原始语义），
					//     提交者正常拨付，恶意争议者一无所获。
					resolution := "honest"
					if k.hasCheatEvidence(ctx, r.TaskId) {
						resolution = "cheat"
					}
					k.resolveDispute(ctx, dispute, resolution)
					continue
				}
				continue
			default:
		...
		// amount * 8500 在 uint64 上对超大 reward
		// 回绕（genesis/迁移路径可绕过经济上限）。85/15 拆分统一走 sdk.Int。
		amountInt := sdkmath.NewIntFromUint64(amount)
		submitterAmountInt := amountInt.MulRaw(int64(types.EdgeAISubmitterRatioBps)).QuoRaw(10000)
		verifierReserveAmountInt := amountInt.Sub(submitterAmountInt) // catch rounding
		if submitterAmountInt.IsNegative() {
			continue
		}
		submitterAmount := submitterAmountInt.Uint64()
		verifierReserveAmount := verifierReserveAmountInt.Uint64()
		// 注意：amount==0（零奖励任务）仍走正常结算路径标记 Done，
		// 否则结果永久滞留 pending 索引成为僵尸条目。

		addr, err := sdk.AccAddressFromBech32(r.Submitter)
		if err != nil {
			continue
		}
		// 拨付 submitter 85%（EdgeAISubmitterRatioBps=8500；原 5% 结算销毁已撤销，份额并入提交者）
	...
	// 退还托管金给任务创建者。
	//
	// 只遍历 open 任务索引的有界轮转批次，不再扫描全量任务表。
	if settledCount < types.MaxTasksPerBlock {
		for _, tid := range k.OpenTaskBatch(ctx, types.MaxOpenTaskScanPerBlock) {
			...
			if task.Reward > 0 {
				...
				if addrErr == nil {
					// task.Reward 虽经 CreateTask 上界校验，但 genesis/迁移
	...
	// Phase 2.5: 验证者预留池过期退还
	//
	// 15% 预留池的领取入口 SubmitVerification 尚无链上 Msg 入口（proto 工具链
	// 待就绪），每笔已结算任务的 15% 托管金连同 verifier_reserve 状态键
	// 永久沉淀为死资金。现规定：结算后 VerifierReserveExpiryBlocks 内未被领取
	// 的预留，按 escrow 归属退还给任务创建者（需求方），并删除预留键。
	// 每区块至多处理 VerifierReserveGCPerBlock 条，游标轮转，成本恒定。
	k.refundExpiredVerifierReserves(ctx)

	// Phase 3: Verifier 多验证者抽检（白皮书行 496-497）
	// 从合格节点中抽取 N 个验证者（默认 3 名），为近期完成任务指派离线重跑
	// 验证；验证者提交结果哈希后由 verifyResultHashMatch 与提交者哈希比对
	// （一致=通过 / 不一致=作弊进入争议）。链上不执行"评分取中位数"——
	// 该伪评分逻辑（sha256%101）已移除，见 scoring.go。
	k.ScoreAndVerify(ctx)
}

// hasCheatEvidence 判断任务是否存在「链上重算哈希与提交哈希不一致」的
// 密码学作弊证据。这是超时自动结案唯一认可的 cheat 依据：
//   - 有 recompute 记录且哈希不一致 → 证据成立；
//   - 无 recompute / 哈希一致 / 无原始结果 → 证据不成立（超时按 honest 结案）。
//
// 证据统一落在这一个位置（RecordRecompute），SubmitVerification 的验证者
// 不一致路径也会先建争议、再写 recompute，保持单一事实来源。
func (k Keeper) hasCheatEvidence(ctx sdk.Context, taskID string) bool {
	...
	return rc.RecomputeHash != "" && rc.RecomputeHash != result.ResultHash
}

// resolveDispute 将争议标记结案（供 BeginBlock 乐观结算与仲裁者 Msg 共用）。
//
// 裁定 cheat 时必须同步——
//  1. 把任务标记为 Cheated（否则任务永久停留在
//     Disputed 状态）；
//  2. 把该任务下全部 pending 结果置为 Rejected（SetResult 会同步清除
//     pending 索引）。否则这些结果成为「僵尸 pending」，每区块白耗
//     MaxTasksPerBlock 结算预算，累积后结算吞吐被持续蚕食。
func (k Keeper) resolveDispute(ctx sdk.Context, d *Dispute, resolution string) {
	...
	if resolution == "cheat" {
		if task, terr := k.GetTask(ctx, d.TaskId); terr == nil && task != nil {
			// cheat 结案必须走完整资金
			// 处置，否则模块账户留下无主余额 —— 仲裁路径回收提交者 85%、退还创建者
			// 15%；BeginBlock 的超时结案路径若只标状态、既不回收也不退款，
			// 创建者的全额托管金永久卡死在模块账户（任务脱离 open 索引后，
			// Phase 2 过期退款再也碰不到它）。
			k.clawbackSubmitterReward(ctx, d.TaskId)
			k.refundVerifierSlice(ctx, task)
			task.Status = types.TaskStatusCheated
			if serr := k.SetTask(ctx, task); serr != nil {
				ctx.Logger().Error("edgeai: SetTask failed on cheat resolution", "err", serr.Error())
			}
		}
		k.rejectResultsForTask(ctx, d.TaskId)
	}
