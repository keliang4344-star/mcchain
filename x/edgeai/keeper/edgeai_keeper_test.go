...
type mockBankCap struct {
	...
	modToMod  []modSend  // 模块 → 模块（作弊回收至安全池等，复用 enterprise_fee_test.go 的 modSend）
}
...
func (m *mockBankCap) SendCoinsFromModuleToModule(_ sdk.Context, from, to string, amt sdk.Coins) error {
...
func TestOpenDispute(t *testing.T) {
	...
	challenger := addrOf(t)
	require.NoError(t, k.SetTask(ctx, &Task{Id: "1", Status: types.TaskStatusOpen}))
	// 争议发起人必须是已指派验证者，且任务已有结果。
	_, verr := k.AssignVerification(ctx, "1", challenger)
	require.NoError(t, verr)
	require.NoError(t, k.SetResult(ctx, &Result{TaskId: "1", Submitter: addrOf(t), Status: types.ResultStatusPending, SubmittedAtBlock: 1}))
...
// TestBeginBlockDisputeTimeoutNoPayout 语义：
// 争议超时的结案方向由链上证据决定 ——
//   - 存在重算证据且与提交哈希不一致 → 判 cheat（不拨款 + slash + 任务 Cheated）；
//   - 无证据 → 判 honest，恶意争议者零收益（零成本开争议不再有利可图）。
func TestBeginBlockDisputeTimeoutNoPayout(t *testing.T) {
	pn := &mockPhonenode{}
	bk := &mockBankCap{}
	k, ctx := setupEdgeaiWith(t, pn, nil, bk)
	arb := addrOf(t)
	...
	// 场景 A：有重算证据（哈希不一致）→ 超时判 cheat，不拨款。
	require.NoError(t, k.SetTask(ctx, &Task{Id: "1", Status: types.TaskStatusOpen}))
	require.NoError(t, k.SetResult(ctx, &Result{TaskId: "1", Submitter: submitter, Status: types.ResultStatusPending, SubmittedAtBlock: 1}))
	require.NoError(t, k.SetDispute(ctx, &Dispute{TaskId: "1", Challenger: challenger, Submitter: submitter, Status: "open", Resolution: "none", OpenedAtBlock: 1}))
	// RecordRecompute 要求任务处于争议态（与链上 OpenDispute 后的状态一致）。
	tk0, _ := k.GetTask(ctx, "1")
	tk0.Status = types.TaskStatusDisputed
	require.NoError(t, k.SetTask(ctx, tk0))
	_, verr := k.AssignVerification(ctx, "1", challenger)
	require.NoError(t, verr)
	require.NoError(t, k.RecordRecompute(ctx, "1", challenger, "evidence_hash_different"))

	ctx = ctx.WithBlockHeight(200)
	k.BeginBlock(ctx)
	require.Empty(t, bk.modToAcct, "有作弊证据的超时争议不应向提交者拨款")
	d, _ := k.GetDispute(ctx, "1")
	require.Equal(t, "cheat", d.Resolution, "有证据的超时争议应判 cheat")
	tk, _ := k.GetTask(ctx, "1")
	require.Equal(t, types.TaskStatusCheated, tk.Status, "cheat 结案应标记任务作弊")

	// 场景 B：无证据争议（主要攻击面）→ 超时判 honest。
	bk2 := &mockBankCap{}
	k2, ctx2 := setupEdgeaiWith(t, &mockPhonenode{}, nil, bk2)
	p2 := types.DefaultParams()
	p2.Arbitrator = addrOf(t)
	k2.SetParams(ctx2, p2)
	submitter2 := addrOf(t)
	require.NoError(t, k2.SetTask(ctx2, &Task{Id: "1", Status: types.TaskStatusOpen}))
	require.NoError(t, k2.SetResult(ctx2, &Result{TaskId: "1", Submitter: submitter2, Status: types.ResultStatusPending, SubmittedAtBlock: 1}))
	require.NoError(t, k2.SetDispute(ctx2, &Dispute{TaskId: "1", Challenger: addrOf(t), Submitter: submitter2, Status: "open", Resolution: "none", OpenedAtBlock: 1}))

	ctx2 = ctx2.WithBlockHeight(200)
	k2.BeginBlock(ctx2)
	d2, _ := k2.GetDispute(ctx2, "1")
	require.Equal(t, "honest", d2.Resolution, "无证据的超时争议应判 honest")
}
...
func TestConsensusCheatDetected_ThreeNodes_OneCheat(t *testing.T) {
	...
	// 验证多数派中恰好一个 valid、另一个被落为 rejected（AllResults 迭代顺序由 KV 决定）
	//
	// 被「任务已结算」跳过的结果不再无限期停留在 pending，
	// 而是落为 Rejected —— SetResult 是 pending 索引唯一的清理点，只跳过不写状态
	// 会让它每轮结算扫描都被重新读到，白耗扫描/结算预算，累积后吞吐随历史任务数衰减。
	validCount := 0
	pendingCount := 0
	rejectedCount := 0
	...
	require.Equal(t, 0, pendingCount, "superseded results must not linger in the pending index")
	require.Equal(t, 2, rejectedCount, "one minority (cheat) + one superseded majority result rejected")
...
// 都不超过 AntiCheatThresholdBps=5000 → 无共识，无人被自动 slash。
//
// 结算侧语义（后）：窗口过后先到者按乐观结算拨付并置任务 Done，
// 落败的那条被落为 Rejected（不再无限期 pending）。
// 本用例只验证「50:50 分歧不触发自动 slash」这一条自动检测边界。
func TestConsensusCheatDetection_TwoNodes_Different(t *testing.T) {
	...
	// 结算：恰好一条 valid（拨付），另一条被落为 rejected（而非僵尸 pending）。
	resA, _ := k.GetResult(ctx, "1", nodeA)
	resB, _ := k.GetResult(ctx, "1", nodeB)
	validCount, rejectedCount := 0, 0
	for _, res := range []*Result{resA, resB} {
		switch res.Status {
		case types.ResultStatusValid:
			validCount++
		case types.ResultStatusRejected:
	...
	require.Equal(t, 1, validCount, "恰好一条结果被乐观结算拨付")
	require.Equal(t, 1, rejectedCount, "落败结果落为 rejected，不得滞留 pending 索引")
	require.Len(t, bk.modToAcct, 1, "只拨付一笔，无重复发放")
}
...
func TestFullLifecycle_MultiSubmitFirstWins(t *testing.T) {
	...
	// 两个结果中恰好一个 valid、另一个落为 rejected（不重复拨付）
	//
	// 落败结果不再滞留 pending 索引，而是落为 Rejected。
	validCount := 0
	pendingCount := 0
	rejectedCount := 0
	...
	require.Equal(t, 0, pendingCount, "superseded results must not linger pending")
	require.Equal(t, 1, rejectedCount, "the losing result is terminally rejected")
}
...
// TestFullLifecycle_DisputeCheatNoPayout：发起争议 → 仲裁裁定 cheat →
// 不再向提交者拨付，且托管资金按 85/15 完整闭合。
//
// 裁定 cheat 时必须同时完成两件事，
// 否则模块账户留下无主余额：
//   - 提交者那份 85% 回收至质押安全池（作恶者的损失变成诚实者的收益）；
//   - 验证者预留那份 15% 退还任务创建者（与 reserve_gc.go 的既有口径一致）。
//
// 注意 assert 的是「不向提交者拨付」，而不是「完全没有任何转账」——
// 后者意味着创建者的托管金永久卡死在模块账户里。
func TestFullLifecycle_DisputeCheatNoPayout(t *testing.T) {
	...
	// 争议裁定 cheat + slash → 不向提交者拨付
	for _, s := range bk.modToAcct {
		require.NotEqual(t, submitter, s.to, "cheat 提交者不得获得任何拨付")
	}
	require.Len(t, pn.slashed, 1)
	require.Equal(t, submitter, pn.slashed[0])

	// 资金完整闭合 —— 85% 回收至安全池，15% 退还创建者，合计 = 托管全额。
	//
	// 注意模块间转账里还会出现「企业结算费」的两笔拆分（40% fee_collector /
	// 60% protocol_treasury，创建任务时即发生），因此这里按目标模块筛选，
	// 而不是假设只有一笔。
	var clawedBack uint64
	clawCount := 0
	...
	require.Equal(t, 1, clawCount, "提交者那 85% 应回收至质押安全池，且只回收一次")
	require.Equal(t, uint64(425), clawedBack, "85% of 500")

	require.Len(t, bk.modToAcct, 1, "创建者应取回验证者预留那 15%")
	require.Equal(t, arbitrator, bk.modToAcct[0].to)
	require.Equal(t, uint64(75), bk.modToAcct[0].amount, "15% of 500")
	require.Equal(t, uint64(500), clawedBack+bk.modToAcct[0].amount,
		"回收 + 退还必须等于托管全额，模块账户不留无主余额")
