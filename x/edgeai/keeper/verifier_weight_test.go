...
// 等概率抽样 rng.Intn(len(addrs)) 会让声誉分只用于扣分与接单
...
// TestVerifierSelectionIsReputationWeighted 锁定核心语义：
// 声誉越高，被抽中做评分者的概率越高。
//
// 构造三名候选，声誉 100 / 50 / 1，期望命中比例趋近 100:50:1。
// 断言取保守区间，只要求严格单调 + 高声誉显著压制低声誉，
// 以免因随机采样波动产生偶发失败。
func TestVerifierSelectionIsReputationWeighted(t *testing.T) {
...
// TestZeroReputationNeverSelected 锁定：声誉被扣至 0 的节点权重归零，
// 只要还存在任何非零声誉候选，它就再也拿不到评分权。
//
// 这是白皮书「反复作弊者自动退出抽检队列」在链上的实际含义——
// 不是靠额外的黑名单，而是权重本身归零。
func TestZeroReputationNeverSelected(t *testing.T) {
...
// TestAllZeroReputationFallsBackToUniform 锁定退化分支：
// 极端情形下全部候选声誉都为 0，抽检机制不能因总权重为零而停摆，
// 此时退化为等概率抽样，仍必须选出验证者。
func TestAllZeroReputationFallsBackToUniform(t *testing.T) {
	nodes := []string{"z1", "z2"}
	k, ctx, _ := setupEdgeaiFull(t, nodes)
...
// TestDefaultReputationParticipates 锁定：从未有过声誉记录的新节点
// 按初始声誉 100 参与加权（GetReputation 对缺失键返回默认分），
// 不能因为「查不到记录」被静默排除在抽检之外。
func TestDefaultReputationParticipates(t *testing.T) {
...
// TestSelectionIsDeterministicWithinBlock 锁定确定性约束在加权路径上
// 依然成立：同一区块上下文重复调用，结果必须逐次相同，
// 否则各验证人写入不同的 Verification 记录 → AppHash 分叉。
func TestSelectionIsDeterministicWithinBlock(t *testing.T) {
...
// TestNoEligibleVerifierReturnsEmpty 锁定边界：候选集为空时返回空串，
// 调用方（BeginBlock 抽检）据此跳过本轮，不得 panic。
func TestNoEligibleVerifierReturnsEmpty(t *testing.T) {
