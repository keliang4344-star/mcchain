...
//
// 这里同时维护两个派生索引，使 BeginBlock 不再需要整库扫描——
...
func (k Keeper) AllDisputes(ctx sdk.Context) []*Dispute {
	...
	for ; it.Valid(); it.Next() {
		...
		if err := k.cdc.Unmarshal(it.Value(), &d); err != nil {
			// 不得在这里 panic。AllDisputes / AllResults /
			// ResultsByTask 都在 BeginBlock 的争议超时结案路径上被调用，一条损坏
			// 条目会让**每个**节点在**每一块**都 panic —— 链停摆，且因为状态没变，
			// 重启后必然再次 panic，属于不可自愈的永久停链。
			// 改为「落日志 + 发事件 + 删除该条目」：删除是全网确定性的（每个节点
			// 都读到同一份损坏数据并做同样处理），链继续出块，损坏可观测、可告警。
			dropCorruptEntry(ctx, store, "dispute", it.Key(), err)
			continue
		}
		dd := d
		out = append(out, &dd)
...
//
// 同步维护 pending 结果索引。BeginBlock 的结算与一致性投票只遍历
...
//
// 结果键为 "result:<taskID>/<submitter>"，同一任务的记录在 KVStore 中
// 本就是连续的，因此这里做一次前缀迭代即可，代价与该任务的提交者数成正比，
// 与全链历史结果总量无关。走 AllResults 全表扫描在规模化下是性能陷阱。
//
...
func (k Keeper) ResultsByTask(ctx sdk.Context, taskID string) []*Result {
	...
	for ; it.Valid(); it.Next() {
		...
		if err := k.cdc.Unmarshal(it.Value(), &r); err != nil {
			// 同 AllDisputes —— 状态损坏必须自愈删除，
			// 不能 panic（BeginBlock 内 panic = 不可自愈的永久停链）。
...
//
// 值格式升级为 "<amount>:<settleHeight>"，记录结算高度，
// 供「预留池过期退还」判断（15% 预留若无 Msg 入口领取就是死资金，
// 永久沉淀在模块账户）。旧格式（纯数字）仍可读取，视为不可过期，平滑兼容。
func (k Keeper) SetVerifierReserve(ctx sdk.Context, taskID string, amount uint64) {
	k.SetVerifierReserveAt(ctx, taskID, amount, uint64(ctx.BlockHeight()))
}

// SetVerifierReserveAt 带结算高度写入预留池（SetVerifierReserve 的显式高度版本）。
func (k Keeper) SetVerifierReserveAt(ctx sdk.Context, taskID string, amount, settleHeight uint64) {
	ctx.KVStore(k.storeKey).Set(verifierReserveKey(taskID),
		[]byte(fmt.Sprintf("%d:%d", amount, settleHeight)))
}

// GetVerifierReserve returns the reserved verifier reward for a task.
// Returns 0 if no reserve exists. 兼容旧格式（纯数字，无高度）。
func (k Keeper) GetVerifierReserve(ctx sdk.Context, taskID string) uint64 {
	amount, _, _ := k.getVerifierReserveEntry(ctx, taskID)
	return amount
}

// getVerifierReserveEntry 解析预留池条目：返回 (amount, settleHeight, ok)。
// settleHeight=0 表示旧格式条目（无高度信息，不参与过期退还）。
func (k Keeper) getVerifierReserveEntry(ctx sdk.Context, taskID string) (uint64, uint64, bool) {
	bz := ctx.KVStore(k.storeKey).Get(verifierReserveKey(taskID))
	if bz == nil {
		return 0, 0, false
	}
	return parseVerifierReserveValue(bz)
}

// parseVerifierReserveValue 解析预留池值字节（供 GC 扫描复用）。
func parseVerifierReserveValue(bz []byte) (uint64, uint64, bool) {
	s := string(bz)
	if i := strings.IndexByte(s, ':'); i >= 0 {
		amount, err1 := strconv.ParseUint(s[:i], 10, 64)
		height, err2 := strconv.ParseUint(s[i+1:], 10, 64)
		if err1 == nil && err2 == nil {
			return amount, height, true
		}
		return 0, 0, false
	}
