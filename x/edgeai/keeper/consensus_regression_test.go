...
// 回归测试
//
// 本文件锁定四项共识与容量性质约束，防止日后被静默改回：
//   ·  共识作弊判定的 slash 写入顺序必须确定（不得依赖 Go map 随机迭代序 → 分叉）
//   ·  声誉衰减每周期至多 -1（不得跨过阈值后每区块都扣 → 百余区块清零）
//   ·  每区块扫描必须有界且能公平轮转（不得整库扫描 → 规模化后停块）
//   ·  按任务取结果必须限定在该任务前缀内（不得走全量结果扫描）
// ---------------------------------------------------------------------------

// TestCheatSlashOrderIsDeterministic 锁定判定顺序：
// 同样的链上状态，反复执行一致性作弊判定，slash 的写入顺序必须字字相同。
//
// 构造：3 个任务，每个任务 5 份 pending 结果 —— 3 份同哈希构成 60% 多数派，
// 另 2 份各持不同哈希构成少数派。少数派分属两个不同的哈希分组，
// 若遍历 hashGroups 这张 Go map 来驱动 SlashIfBad，两名少数派谁先被罚
// 每次运行都可能不同；各验证者写入的 slash 记录顺序不一致 → AppHash 分叉。
func TestCheatSlashOrderIsDeterministic(t *testing.T) {
...
// TestMajorityTieIsDeterministic 锁定多数派并列时的裁决确定性：
...
// TestDecayAtMostOncePerPeriod
// 越过无贡献阈值后，每个衰减周期至多扣 1 分，而不是此后每个区块都扣。
func TestDecayAtMostOncePerPeriod(t *testing.T) {
	...
	// 紧接着的 50 个区块内不得再扣（这正是每周期至多衰减一次的核心）：
	// 每个区块扣 1 分会让满分节点在百余区块内被清零。
...
// TestNoDecayForNodeWithoutContribution 从未贡献过的新节点不参与衰减。
func TestNoDecayForNodeWithoutContribution(t *testing.T) {
	k, ctx, _ := setupEdgeaiFull(t, nil)
	require.NoError(t, k.SetReputation(ctx, &Reputation{
		NodeAddr:              "fresh",
		Score:                 types.DefaultReputationScore,
...
// TestBoundedScanRotates 锁定轮转语义：
// 每次至多取 limit 条；连续调用可无重复地覆盖全集；走到末尾自动回到开头。
func TestBoundedScanRotates(t *testing.T) {
	k, ctx, _ := setupEdgeaiFull(t, nil)
...
// TestPendingBatchKeepsGroupsComplete 锁定分组完整性：
// 达到任务数上限时必须停在「下一个任务的第一条」上，
// 已纳入的任务必须携带其全部提交者，否则一致性投票会误判多数派。
func TestPendingBatchKeepsGroupsComplete(t *testing.T) {
	k, ctx, _ := setupEdgeaiFull(t, nil)
...
// TestPendingIndexDropsSettledResults 结果一旦离开 pending，
// 必须同步从索引摘除，否则轮转会被历史结果永久占满。
func TestPendingIndexDropsSettledResults(t *testing.T) {
	k, ctx, _ := setupEdgeaiFull(t, nil)
...
// TestOpenTaskIndexTracksStatus open 任务索引随状态变化增删。
func TestOpenTaskIndexTracksStatus(t *testing.T) {
	k, ctx, _ := setupEdgeaiFull(t, nil)
...
// TestDoneRingIsBounded 近期完成任务环长度恒定，不随历史任务累积增长。
func TestDoneRingIsBounded(t *testing.T) {
	k, ctx, _ := setupEdgeaiFull(t, nil)
...
// TestResultsByTaskIsScoped
// 按任务取结果只在该任务前缀内迭代，不得触发全量结果扫描。
func TestResultsByTaskIsScoped(t *testing.T) {
	k, ctx, _ := setupEdgeaiFull(t, nil)
