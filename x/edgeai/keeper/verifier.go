...
//
// 白皮书载明「验证者遴选按声誉加权」
// （Verifier selection is reputation-weighted）。朴素实现是等概率均匀抽样，
...
//     保证抽检机制本身不会因此停摆。
//
// 确定性：候选顺序来自 staking 的 power 索引（确定），
...
func (k Keeper) SelectVerifierNode(ctx sdk.Context) string {
	...
	// 确定性随机源（见 rand.go），禁止 math/rand 全局源。
...
//
// 候选集取自定长「近期完成任务环」（DoneTaskRingSize 条），
...
func (k Keeper) SubmitVerification(ctx sdk.Context, taskID, verifierAddr, verifierResultHash string) error {
		...
		if existing == nil {
			...
			if terr == nil && task != nil {
				...
				if derr := k.SetDispute(ctx, d); derr == nil {
					// 把验证者哈希写入 recompute 记录，
					// 作为超时自动结案的唯一证据源（hasCheatEvidence）。
					// 争议已先建好，RecordRecompute 的「需争议态」检查可通过。
					if rerr := k.RecordRecompute(ctx, taskID, verifierAddr, verifierResultHash); rerr != nil {
						ctx.Logger().Error("edgeai: record recompute evidence failed",
							"task_id", taskID, "err", rerr.Error())
					}
	...
	// 从该任务的验证者奖励预留池领取 15%。
	//
	// 预留池为空时不得回退到固定的兜底奖励（1 MC）：
	// 这笔钱没有任何托管背书——edgeai 模块账户里当时只剩别的任务的托管金，
	// 于是「兜底奖励」实际是在挪用其他任务的托管资金，任务越多亏空越大，最终某个
	// 任务结算时会因余额不足而失败。无托管即无奖励，这里必须直接跳过。
	reserve := k.GetVerifierReserve(ctx, taskID)
	if reserve == 0 {
		ctx.EventManager().EmitEvent(
			sdk.NewEvent("edgeai.VerifierRewardSkipped",
				sdk.NewAttribute("task_id", taskID),
				sdk.NewAttribute("verifier", verifierAddr),
				sdk.NewAttribute("reason", "no escrowed verifier reserve for this task"),
			),
		)
		return nil
	}
	...
	// reserve 可能来自任务结算的 15% 预留池（由 task.Reward 推导），
...
func (k Keeper) SampleAndVerify(ctx sdk.Context) {
	// 注意：不得调用 rand.Seed(...) 播种 math/rand 全局源，
	// 该做法自 Go 1.20 起已废弃且在共识路径下不安全（全局源为进程共享，
	// 播种与取值之间会被其它 goroutine 打断，各节点序列不一致 → 分叉）。
	// 本实现改为在各取值点由区块数据派生本地 *rand.Rand（见 rand.go），
	// 无需也不允许在此播种任何全局状态。
