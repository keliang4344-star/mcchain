...
//
// 洗牌必须使用由区块数据派生的确定性随机源（见 rand.go），
...
//
// 不采用"中位数阈值"伪评分：sha256(taskID:verifier)%101 生成
...
func (k Keeper) ScoreAndVerify(ctx sdk.Context) {
	...
	// 抽检候选取自定长「近期完成任务环」（DoneTaskRingSize 条），
	// 一次读取供本轮全部验证者复用。若对每个验证者各做一次 AllTaskIDs
