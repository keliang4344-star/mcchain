...
// BeginBlock 每块执行三项有界工作：
//
//  1. EnsureReleaseScheduleStart：把释放窗口起点钉在链上首个区块时间（幂等）。
//  2. DrainDeferred：按固定预算（types.DeferredDrainBudget 条/块）把延后账本里
//     过期的返佣补发入 pending，严格 FIFO。成本恒定，不会随延后量线性增长。
//  3. ResetDailyCaps：增量回收过期的日计数器（纯垃圾回收，不影响限额正确性）。
//
// 另会在预算跌破低水位时发出一次按日去重的 BudgetLow 告警事件。
func (am AppModule) BeginBlock(ctx sdk.Context, _ abci.RequestBeginBlock) {
	am.keeper.EnsureReleaseScheduleStart(ctx)
	am.keeper.DrainDeferred(ctx)
	am.keeper.ResetDailyCaps(ctx)
	am.keeper.MaybeEmitBudgetLowAlert(ctx)
}
