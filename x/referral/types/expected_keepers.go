...
type PhonenodeKeeper interface {
	...
	// IsActiveNode returns true if the address is a full mining node:
	// registered + attestation valid + heartbeat within the offline grace window.
	// 10 代分级推荐：挖矿节点享满 10 代收益，非节点只享前 5 代。
	IsActiveNode(ctx sdk.Context, addr string) bool
}
