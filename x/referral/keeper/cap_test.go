...
// TestCheckDailyCaps_NoOverflowBypass 是核心回归：
//
// 朴素实现用 `used + bonus.Uint64` 与上限比较。攻击者只要让
...
func TestCheckDailyCaps_NoOverflowBypass(t *testing.T) {
	...
	// bonus = 100 时，朴素实现 used+bonus 会回绕为 89（远小于 cap=1000）→ 放行；
	// 新实现在任意精度上比较 → 必须拒绝。
...
// TestResetDailyCaps_PrunesOnlyExpired 是仅清理过期条目的回归测试。
//
// 朴素实现在日边界那一个区块里无界删除全部 per-user 键，用户上量后会拖垮
// BeginBlock；新实现改为每块按预算增量清理过期键。这里验证两点：
//  1. 清理只针对**过期日**的键，当日计数器绝不能被抹掉（否则当天可重复领满额度）；
//  2. 在非日边界高度调用也安全（新实现每块都跑，不再只在边界跑）。
func TestResetDailyCaps_PrunesOnlyExpired(t *testing.T) {
	bank := newMockRefBank()
	k, ctx := newReferralKeeper(t, bank)
	inviter := sdk.AccAddress(secp256k1.GenPrivKey().PubKey().Address()).String()
	...
	// 日边界口径已统一为「区块时间 / 86400」（与 x/depin 同源，见 internal/daywindow），
	// 因此推进一天必须改区块时间，而不是改区块高度 —— 高度与日界已经解耦。
	day0 := ctx.WithBlockTime(time.Unix(0, 0).UTC())
	k.RecordDailyCapUsage(day0, inviter, sdkmath.NewInt(900))
	require.Error(t, k.CheckDailyCaps(day0, inviter, sdkmath.NewInt(200)), "当天额度应已接近用尽")

	// 第 2 天：键换成新的一天，额度自然重置（与是否清理无关）。
	day1 := ctx.WithBlockTime(time.Unix(2*86400, 0).UTC())
	require.NoError(t, k.CheckDailyCaps(day1, inviter, sdkmath.NewInt(900)))
	k.RecordDailyCapUsage(day1, inviter, sdkmath.NewInt(300))

	// 在非日边界高度反复清理：过期键被回收，当日计数器必须原样保留。
	for i := 0; i < 5; i++ {
		require.NotPanics(t, func() { k.ResetDailyCaps(day1) })
	}
	require.Error(t, k.CheckDailyCaps(day1, inviter, sdkmath.NewInt(800)),
		"清理不得抹掉当日已用额度（300+800 > 1000 必须被拒）")
	require.NoError(t, k.CheckDailyCaps(day1, inviter, sdkmath.NewInt(700)))
}
