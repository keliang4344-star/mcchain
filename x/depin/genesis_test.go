...
// genesis state (with the InitialPool / RewardDenom params) must validate.
func TestGenesis(t *testing.T) {
	genesisState := types.DefaultGenesis()
	require.NoError(t, genesisState.Validate())
}

// （五池模型）：depin 不再自铸。InitGenesis 仅 SetParams；
...
func TestInitGenesisDoesNotMint(t *testing.T) {
	...
	// depin 不再自铸，bank 不应被调用铸造。
	require.Empty(t, bank.mintedModule, "depin must not mint")
	// 参数应已正确写入。
