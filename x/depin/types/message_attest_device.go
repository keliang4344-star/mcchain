...
func (msg *MsgAttestDevice) ValidateBasic() error {
	...
	// challenge 会进入留痕键、一次性消费索引键与 attestation 历史，
	// 必须在无状态层就给出长度上界（keeper 侧另有同口径防线）。
	if msg.Challenge == "" {
		return sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "challenge must not be empty")
	}
	if len(msg.Challenge) > MaxAttestChallengeLength {
		return sdkerrors.Wrapf(sdkerrors.ErrInvalidRequest,
			"challenge too long: %d > %d", len(msg.Challenge), MaxAttestChallengeLength)
	}
	return nil
}
