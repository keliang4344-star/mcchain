...
// 释放规则：
//   每日释放上限 = 金库当前余额 / 剩余天数
//   默认周期 4015 天（≈ 11 年），即设备激励金库线性摊薄释放窗口。
//
...
const (
	// DefaultVaultDays 默认设备激励金库线性摊薄释放的总天数。
	// 4015 天 ≈ 11 年（4015 ÷ 365.25 ≈ 10.99）。这是设备挖矿奖励金库自身的释放窗口，
	// 与白皮书另述的「节点资本津贴十二年底线」（4.675 亿设备池）及「质押滴灌十二年底线」
	// （1.5 亿池）是三套相互独立的周期，请勿混为一谈。
	DefaultVaultDays = 4015

	// DaySeconds 一天的秒数。
	//
	// 数值来源统一到 internal/daywindow，不再由各模块自持一份。
	// referral 若自持「14400 块/日（按 6 秒）」而链实际 4 秒出块，两套口径
	// 让同一笔经济流的上下游落在不同窗口里；现在秒/块两个口径都只有一处定义。
	DaySeconds = daywindow.SecondsPerDay
)
...
func (k Keeper) GetOrInitReleaseVault(ctx sdk.Context) (*ReleaseVault, error) {
	...
	// Int.Uint64 在余额超出 uint64 或为负时直接 panic（DeliverTx panic = 全网停机）。
