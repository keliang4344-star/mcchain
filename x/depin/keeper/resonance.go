...
// 朴素实现先用未钳制的 contributionQuality 算出 qualityFactor，之后才 clamp，
...
//
// 返回 sdk.Dec 而非 float64，保证负载因子逐位可复现。
