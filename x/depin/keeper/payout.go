...
// PayoutReward 从 DePIN 模块账户（生态池，由 tokenomics 在 InitGenesis 一次性拨付）
// 向 addr 拨付 amount 个 reward denom（umc）。
//
// 经济约束：本函数不铸造、不突破总量硬顶（1B MC），仅从池内划拨；
// 池余额不足时返回错误，由调用方决策（通常是拒绝拨付、保留结果）。
//
// 这是 的拨付入口：x/edgeai 在任务结果通过争议期后调用本函数，
...
func (k Keeper) PayoutReward(ctx sdk.Context, addr sdk.AccAddress, amount uint64) error {
	...
	// amount 是导出 API 的 uint64 入参，int64 在超 MaxInt64 时会回绕为负，
