...
// and to route slashed stake. Iron rule: this module MUST NOT mint MC —
...
// StakingKeeper slashing 所需的 staking 接口子集（仅取方法签名，避免 x/phonenode 依赖具体实现）。
//
...
type StakingKeeper interface {
	...
	// (see keeper.GetVerifierNodes ): we enumerate the small validator
	// set and look each one up in phonenode, instead of scanning every
	// registered device.
	GetBondedValidatorsByPower(ctx sdk.Context) []stakingtypes.Validator
}

// DepinReleaseKeeper 暴露设备激励池（depin 模块）的「每日线性释放额度」闸门。
//
// 节点资本津贴 30 MC/节点/日 直接从 depin 模块账户
// 转出，却完全绕过了 depin 的日释放上限——设备池的线性释放曲线因此形同虚设，
// 节点规模上量后津贴一条腿就能把 7 亿设备池提前抽干。津贴必须与 DePIN 任务奖励
// 共用同一个日额度闸门，才谈得上「释放节奏可控」。
//
// 只用 sdk 原生类型，x/phonenode/types 不 import x/depin/types，避免循环依赖。
...
// SlashingKeeper 所需的 slashing 接口子集，只保留 Jail（吊销出块权）。
//
