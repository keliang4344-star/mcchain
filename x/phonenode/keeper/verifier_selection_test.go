...
// 回归测试（phonenode）
//
//	节点以账户地址登记，罚没与验证者遴选必须按同字节换算出验证人地址；
//	          朴素实现用 ValAddressFromBech32 解析账户地址，恒失败 →
//	          验证人作恶不掉一分质押、验证者集合恒为空。
//	验证者遴选必须遍历「验证人集合」（受 MaxValidators 封顶），
//	          而不是遍历「全量注册设备」。
// ---------------------------------------------------------------------------
...
// TestVerifierNodesAcceptAccountAddress
// 节点用账户地址（mc1...）登记，仍必须被识别为合格验证者。
// 若在此处对每个节点解码失败，本函数将恒返回空集，
// EdgeAI 第三阶段抽检从未真正运行。
func TestVerifierNodesAcceptAccountAddress(t *testing.T) {
...
// TestSlashHitsValidatorRegisteredByAccountAddress 锁定罚没侧：
// 以账户地址发起 slash，必须真正扣到该验证人的自质押。
func TestSlashHitsValidatorRegisteredByAccountAddress(t *testing.T) {
	...
	require.Equal(t, int64(200), stk.removed.Int64(),
		"账户地址形态的验证人节点必须被真正罚没自质押（朴素实现在此静默跳过）")
...
// TestVerifierSelectionIsBoundedByValidatorSet
// 遴选成本只与验证人集合规模相关，与注册设备总量无关。
// 这里注册大量普通设备，验证者集合仍只含那一个合格验证人。
func TestVerifierSelectionIsBoundedByValidatorSet(t *testing.T) {
	val, accAddr := bondedValidator(t, 40_000_000_000)
	...
	// 注册 300 台普通设备（非验证人）——朴素实现会把它们全部读进内存逐个筛，
	// 现实现根本不会碰到它们。
	for i := 0; i < 300; i++ {
