...
//
// 本函数由 EdgeAI 的 BeginBlock 第三阶段
// 每区块调用。朴素实现先 AllNodes 把**全量注册设备**读进内存，再逐个问 staking
...
// GovAuthority 返回治理模块账户地址——唯一有权强制改写验证人状态的主体。
...
//
// 授权模型：本方法不得零鉴权 —— 否则 msg_server 直接把 msg.NodeId
// 透传进来——任何账户都能把任意节点（包括刚被罚没 jail 的自己）改回 active，
// 从而恢复 30 MC/日 的节点资本津贴，罚没等于没罚。规则如下：
//
//   - 治理模块账户：三态皆可设，包括无条件解除 jail；
//   - 节点本人：可在 active ⇄ inactive 之间自愿切换；
//   - 已被 jail 的节点本人：只能走「冷却期届满 + 当前认证有效」这条可审计通道
//     恢复为 active，做不到即时自助复原；
//   - 任何人都不能把别人的节点改状态，也不能自我 jail。
func (k Keeper) UpdateVerifierStatus(ctx sdk.Context, signer, nodeID, status string) error {
	if !types.IsValidVerifierStatus(status) {
		return types.ErrInvalidVerifierStatus
	}
	...
	if signer != GovAuthority() {
		if signer != nodeID {
			return sdkerrors.Wrapf(sdkerrors.ErrUnauthorized,
				"signer %s may not change verifier status of node %s", signer, nodeID)
		}
		if status == types.VerifierStatusJailed {
			return sdkerrors.Wrap(sdkerrors.ErrUnauthorized,
				"jailed status is protocol-imposed and cannot be self-assigned")
		}
		if st.VerifierStatus == types.VerifierStatusJailed {
			// 罚没后的自助恢复通道：冷却期未过、或认证未重新生效，一律拒绝。
			if k.InSlashCooldown(ctx, nodeID) {
				return sdkerrors.Wrap(types.ErrSlashCooldown,
					"node is jailed and still in slash cooldown")
			}
			if !k.IsAttested(ctx, nodeID) {
				return sdkerrors.Wrap(sdkerrors.ErrUnauthorized,
					"node is jailed; re-attest with a valid attestation before restoring")
			}
		}
	}
