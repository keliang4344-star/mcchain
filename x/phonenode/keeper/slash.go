...
// 朴素实现直接用 ValAddressFromBech32 解析节点地址，于是：
...
//
// 硬约束：本函数绝不调用 tokenomics.MintCoins，minted_supply 不变（总量硬顶不受 slash 影响）。
func (k Keeper) SlashIfBad(ctx sdk.Context, addr, reason string, penaltyBps uint32) error {
	...
	// 1.1 同步把本模块 NodeState.VerifierStatus 置为 jailed，使节点资本津贴
	// 发放（DistributeNodeCapitalAllowances 的 jail/inactive 过滤 + IsAttested 闸口）
	// 能正确拦截被罚没/认证吊销的作恶节点，避免其继续每日领取 30 MC。
	//
	// 这里必须是 jailed 而不是 inactive：inactive 属「自愿停役」，节点本人可随时
	// 自行切回 active；而罚没是协议强制结果，只能沿「冷却期届满 + 重新认证有效」
	// 这条可审计路径恢复（详见 UpdateVerifierStatus 的授权模型），否则作恶节点
	// 被罚后下一个区块就能自助复原、继续领取 30 MC/日，罚没等于没罚。
	if st, serr := k.GetNode(ctx, addr); serr == nil {
		st.VerifierStatus = types.VerifierStatusJailed
		if setErr := k.SetNode(ctx, st); setErr != nil {
			ctx.Logger().Error("phonenode: set node verifier status on slash failed",
				"node", addr, "err", setErr.Error())
		}
		// 被 slash 的节点必须退出离线检测队列。
		//
		// SetNode 在 LastProofBlock 未变化时只会等价重写索引位（幂等），不会移除它。
		// 若不在此处显式出队，该条目会永久滞留在心跳索引头部，从此每个区块被
		// 反复扫到、反复读 attestation（此时已吊销）与节点状态却什么都不做，
		// 把 MaxOfflineScanPerBlock 的扫描预算吃干净，真正新离线的节点排队
		// 无限延长——而节点资本津贴在此期间照发，等于给「掉线不亏」开口子。
		//
		// 回归验证：注释掉本行后 TestDetectOfflineDrainsProcessedEntries 由
		// 640 降为 512（128 条预算被僵尸条目白吃），可作为该修复的守门测试。
		//
		// 节点在冷却期届满后重新 attest 时，会经 SetNode 以新的 LastProofBlock
		// 重新入队，检测能力随之恢复。
		k.removeHeartbeatIndex(ctx, addr, st.LastProofBlock)
	}

	// 1.5 写入 slash 冷却：被 slash 后限时禁止再认证，
