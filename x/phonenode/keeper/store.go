...
// setHeartbeatIndex 写入节点在指定心跳高度上的索引位（幂等）。
//
// 索引与主记录同事务更新，二者不会漂移。高度 <= 0 表示「尚未产生心跳」，
// 此时不建索引位：be8(0) 会排在索引最头部，会被离线检测当成最早的到期候选，
// 等于让新注册设备立刻进入待判离线队列。
func (k Keeper) setHeartbeatIndex(ctx sdk.Context, addr string, lastProofBlock int64) {
	if lastProofBlock <= 0 {
		return
	}
	ctx.KVStore(k.storeKey).Set(types.HeartbeatIndexKey(lastProofBlock, addr), []byte{1})
}

// removeHeartbeatIndex 摘除节点在指定心跳高度上的索引位。
//
// 这个「出队」动作是离线检测容量性质的支点。索引头部即最早心跳区段，
// 若处理过的条目不出队，它会永久滞留在头部，每个区块被反复扫到、反复读
// attestation 与 node 状态，却什么都不做——把每块预算（MaxOfflineScanPerBlock）
// 全部吃掉，真正新离线的节点排队无限延长，而津贴在此期间照发。
// 因此凡是「已经不需要再检测」的条目（被 slash、attestation 失效、节点已删）
// 都必须经由本函数离开索引。
func (k Keeper) removeHeartbeatIndex(ctx sdk.Context, addr string, lastProofBlock int64) {
	if lastProofBlock <= 0 {
		return
	}
	ctx.KVStore(k.storeKey).Delete(types.HeartbeatIndexKey(lastProofBlock, addr))
}

// SetNode 持久化节点状态（upsert）。
//
// 同步维护「按最近心跳高度排序」的节点索引。本函数是节点状态的唯一
// 写入点（注册 / 心跳 / attestation / 验证者状态变更全部经此），索引与主记录
// 同事务更新，不会漂移。有了它，离线检测才能只看真正超时的那一小段区间，
// 而不是每个区块把全量节点读进内存。
//
// 注意：本函数在 LastProofBlock 未变化时不会移除既有索引位
// （只做等价重写，幂等）。因此「惩罚/吊销后让节点退出检测队列」必须由调用方
// 在 SetNode 之后显式调用 removeHeartbeatIndex 完成——见 SlashIfBad。
func (k Keeper) SetNode(ctx sdk.Context, st *NodeState) error {
	...
	if old := store.Get(nodeKey(st.Address)); old != nil {
		...
		if json.Unmarshal(old, &prev) == nil && prev.LastProofBlock != st.LastProofBlock {
			k.removeHeartbeatIndex(ctx, st.Address, prev.LastProofBlock)
		}
	}

	store.Set(nodeKey(st.Address), bz)
	k.setHeartbeatIndex(ctx, st.Address, st.LastProofBlock)
	return nil
}
...
// IsActiveNode reports whether the address is a full mining node as defined by
// the whitepaper's tiered referral model: registered + attestation valid +
// heartbeat within the offline grace window.
//
// 挖矿节点判定（白皮书推荐 10 代分级）：
//   - 已注册（HasNode）；
//   - 设备认证有效（AttestationStatusValid，30 天窗口内）；
//   - 最近心跳在离线宽限（OfflineGraceBlocks）内——离线超窗视为非活跃。
//
// 与 VerifierMinStake 无关：挖矿节点不要求验证人抵押，只要求真实在网。
func (k Keeper) IsActiveNode(ctx sdk.Context, addr string) bool {
	st, err := k.GetNode(ctx, addr)
	if err != nil || st == nil || !st.Registered {
		return false
	}
	att, ok := k.GetAttestation(ctx, addr)
	if !ok || att.Status != types.AttestationStatusValid || att.IsExpired(ctx.BlockTime().Unix()) {
		return false
	}
	params := k.GetParams(ctx)
	if params.OfflineGraceBlocks > 0 &&
		ctx.BlockHeight()-st.LastProofBlock > params.OfflineGraceBlocks {
		return false
	}
	return true
}
