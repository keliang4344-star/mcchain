...
// TestSlashCooldownBlocksReAttest 锁定非验证人 slash 冷却细则：
...
func TestSlashCooldownBlocksReAttest(t *testing.T) {
	...
	// nonce / device_id_hash 现在有长度与字符集闸门（防 KVStore 键膨胀），
	// 测试数据必须落在合法区间内（nonce ≥8 可打印 ASCII，device_id_hash 为 64 位小写 hex）。
	nonce2, nonce3 := "nonce-aaaa-0002", "nonce-aaaa-0003"
	hash2, hash3 := strings.Repeat("a1", 32), strings.Repeat("b2", 32)
	sig2, _ := devPriv.Sign([]byte(hash2 + "|" + nonce2))
	sig3, _ := devPriv.Sign([]byte(hash3 + "|" + nonce3))

	// 冷却期内再认证应被拒（在验签之前短路）
	err = k.SubmitAttestation(ctx, addr, "root2", nonce2, hash2, devPubHex, hex.EncodeToString(sig2))
	...
	err = k.SubmitAttestation(ctx, addr, "root3", nonce3, hash3, devPubHex, hex.EncodeToString(sig3))
	require.NoError(t, err)
	require.True(t, k.IsAttested(ctx, addr))
}
