...
func TestDefaultGovernanceHandoverConfig(t *testing.T) {
	...
	// 默认必须是「可达」状态——开启流程并把治理主体锚定为链上治理模块账户。
	// 否则白皮书承诺的渐进治理移交给在代码上 100% 不可达（死开关）。
	require.True(t, cfg.Enabled, "默认必须开启移交流程，否则该能力不可达")
	require.Equal(t, keeper.DefaultGovernorAddress(), cfg.CurrentGovernor,
		"默认治理主体应为链上治理模块账户（全网确定性派生，不引入特权私钥）")
...
// 零值配置（Enabled=false 且 CurrentGovernor==""）必须被归一化为默认可达配置。
// 该组合不可能是任何有意义的治理意图，只可能是修复前遗留的未初始化状态。
func TestHandoverZeroValueConfigNormalized(t *testing.T) {
	k, ctx := setupHandoverKeeper(t)

	k.SetGovernanceHandoverConfig(ctx, keeper.GovernanceHandoverConfig{
		Enabled:        false,
		TimelockBlocks: 100,
	})

	cfg := k.GetGovernanceHandoverConfig(ctx)
	require.True(t, cfg.Enabled, "零值配置应归一化为默认（开启）")
	require.Equal(t, keeper.DefaultGovernorAddress(), cfg.CurrentGovernor)
}

// 显式关闭移交时（Enabled=false 且已配置治理主体），发起移交必须失败。
func TestInitiateHandoverDisabled(t *testing.T) {
	k, ctx := setupHandoverKeeper(t)

	cfg := k.GetGovernanceHandoverConfig(ctx)
	cfg.Enabled = false
	cfg.CurrentGovernor = newGovernorAddr()
