...
// 治理主体为空时不再落入「无人可发起」的死状态，而是确定性回落到链上治理模块账户。
// 修复前 CurrentGovernor=="" 会让 AssertGovernanceAuthority 无条件拒绝一切调用，
// 叠加 Enabled=false 使整个移交能力 100% 不可达（承诺与实现不一致）。
func TestMsgServerHandoverEmptyGovernorFallsBackToGovModule(t *testing.T) {
	k, ctx := setupHandoverKeeper(t)
	ms := keeper.NewMsgServerImpl(*k)
	...
	require.Equal(t, keeper.DefaultGovernorAddress(), k.GetGovernanceHandoverConfig(ctx).CurrentGovernor,
		"空治理主体应回落为链上治理模块账户")

	// 治理模块账户无私钥，因此普通账户冒充治理主体必须被拒（唯一通道是治理提案）。
