...
- 费用接收方（本链已做特殊分账，见 `x/tokenomics/keeper/gas_rebate.go`）：每笔 gas 费按基点拆分 ——
  **7%（`GasBurnRatioBps=700`）永久销毁转入黑洞地址**（通缩）、**10%（`GasRebateRatioBps=1000`）回流质押安全池**补贴诚实验证人、
  余下约 83% 进入区块 proposer 奖励池（FeeCollector）。这与白皮书销毁政策表（`GasBurnRatioBps = 700`，手续费的 7% 转入黑洞）口径一致，不复用 SDK 默认全文进入 proposer 的行为。

## 4. Gas 上限与DoS 防护

- 区块 `gas` 上限由创世 `consensus_params.block.max_gas` 控制；若创世未显式设置（历史默认 -1 = 无界），
  `InitChainer` 会在链启动首块将其 fail-closed 钳制为 **100,000,000**（`app/app.go` 的 `DefaultBlockMaxGas`），
  杜绝区块 gas 无界导致的出块失控 DoS。`deploy/init.sh` 与 `server_setup.sh` 均已显式写入该上限。单笔交易的 gas 不得超过区块上限。
...
| 分配方向 | 比例 | 常量 | 说明 |
|---|---|---|---|
| **Burn（销毁）** | 50%（= 成交额 0.15%） | `FeeBurnBps = 5000` | 转入黑洞地址，永久退出流通，通缩效应 |
| **LP（流动性提供者）** | 50%（= 成交额 0.15%） | `FeeLPBps = 5000` | 留存池中储备，按 LP 份额归流动性提供者 |
| **Treasury（国库）** | 0% | `FeeTreasuryBps = 0` | 国库不从 swap 手续费分成，另由企业结算费等渠道注资 |
...
| 费用去向 | 进入区块 Proposer 奖励池 | 50% 销毁 + 50% LP（国库不分成） |
